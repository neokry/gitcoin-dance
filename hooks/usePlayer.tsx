...
export default function usePlayer(
  playerIndex: number,
  round: number
): { funds: number; getPlayerFunds: () => void } {
  ...
  const getRoundScores = (queryRound: number) => {
    ...
    const lastRoundData = roundHistory.find(
      (x) => x.round.toNumber() === queryRound + 1
    );
    let position = -1;
    if (lastRoundData) {
      position = lastRoundData.bracketWinners.findIndex(
        (x) => x == playerIndex
      );
    }
